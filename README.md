...
After implementation, invoke `feature-close` to actively compare every applicable Task, Feature, and Milestone Acceptance requirement with the current implementation and verification/integration evidence, whether checked or unchecked. It runs available checks to fill evidence gaps, marks proven items `[x]`, clears unsupported marks with reasons, and reports each item's evidence and remaining blockers. Once pre-integration checks pass, it asks you to choose an applicable next action: merge the exact branch or PR/MR, publish a PR/MR, or keep the work for later. Items that depend on integration remain unchecked until verified afterward. After verified integration it asks about cleanup, preserving separate confirmation for a specific merge, branch/worktree deletion, or discard.
