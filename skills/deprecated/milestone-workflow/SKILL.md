...
description: Deprecated archive of the former stateful Milestone coordinator. Do not use for new work; use shared Milestone artifacts, Feature ownership, the configured tracker, and read-only waypoint-workflow navigation.
---

> [!WARNING]
> Deprecated snapshot. Use `milestone-planning` and `waypoint-workflow` for new work. This file is retained only for migration reference.
