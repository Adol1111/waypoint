---
name: task-state
description: Deprecated archive of the former task checkpoint skill. Do not use for new work; use the configured external tracker or explicitly initialized local-work-tracker for operational state.
---

> [!WARNING]
> Deprecated snapshot. Use the configured external tracker or `local-work-tracker` for new work. This file is retained only for migration reference.
