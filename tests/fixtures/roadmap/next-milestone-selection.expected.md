...
- The result starts from completed delivery, product gaps, Future Directions, user or operational evidence, and the full backlog rather than inventing a task list.
- Goal-shaped uncertainty stays out of the task backlog.
- Duplicate, superseded, and already completed backlog items are reconciled before new items are added.
- Scores use the repository model or the fallback holistic `0–10` anchors and one evidence-based rationale; only changed evidence rewrites a score.
- The leading candidates are compared by score, dependency, readiness, and stage proof without pretending that score is dependency order.
- The requested five-or-six-task batch is a soft planning boundary, not a fixed timebox or an excuse to fragment work.
- One backlog candidate that expands into three atomic tasks consumes three places; the second candidate's two tasks consume two more.
